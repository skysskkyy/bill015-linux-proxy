...
def normalize_responses_request(body: dict[str, Any], request_headers: dict[str, str] | None = None, cfg: Settings = settings) -> Turn:
    ...
    return Turn(
        ...
        previous_response_id=str(cleaned["previous_response_id"]) if cleaned.get("previous_response_id") else None,
        prompt_cache_key=str(cleaned["prompt_cache_key"]) if cleaned.get("prompt_cache_key") else None,
        request_kind=kind,
